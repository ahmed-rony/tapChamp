  ...
  useEffect(() => {
    if (timerStarted) {
      if (points >= requiredTaps && !adShown) {
        setMessage('Congratulations! You Beat Level 8!');
        congratsSound.play(success => {
          if (!success) {
            console.log('Congrats sound did not play');
          }
        });
        if (!unlockedLevels.includes(9)) {
          setUnlockedLevels([...unlockedLevels, 9]);
        }
        showInterstitialAd();
        stopTimer();
        ...
        {addClosed && message && (
          <Modal visible={true} animationType="slide" transparent={true}>
            <View style={styles.modalContainer}>
              ...
              {message === 'Congratulations! You Beat Level 8!' && (
                <ConfettiCannon
                  count={200}
                  origin={{x: -20, y: 0}}
                  explosionSpeed={400}
