...
const LVLFour = ({navigation}) => {
  ...
  //new device emmiter
  //useEffect(() => {
  // const subscription = DeviceEventEmitter.addListener('livesUpdated', () => {
  //  // Actions to perform when lives are updated
  // You can add logic here to update the component based on the new lives count
  //  console.log("Lives were updated!");
  // });
  ...
  useEffect(() => {
    if (timerStarted) {
      if (points >= requiredTaps && !adShown) {
        setMessage('Congratulations! You Beat Level 4!');
        ...
        if (!unlockedLevels.includes(5)) {
          setUnlockedLevels([...unlockedLevels, 5]);
        }
        showInterstitialAd();
        stopTimer();
  ...
  const handleButtonClick = buttonType => {
    ...
    if (
      points < requiredTaps &&
      (lastButtonPressed === null || lastButtonPressed !== buttonType)
    ) {
      setPoints(points + 1);
        ...
        {addClosed && message && (
          <Modal visible={true} animationType="slide" transparent={true}>
            <View style={styles.modalContainer}>
              ...
              {message === 'Congratulations! You Beat Level 4!' && (
                <ConfettiCannon
                  count={200}
                  origin={{x: -20, y: 0}}
                  explosionSpeed={400}
