...
import {interstitialAdKeyAndroid} from './AdConfig';
import useAdRemovalPurchase from './useAdRemovalPurchase'; //add

Sound.setCategory('Playback');

if (Platform.OS === 'android') {
  var congratsSound = new Sound(require('../android/sounds/CongratsT.mp3'), error => {
    console.log('🚀 ~ congratsSound=newSound ~ error:', error);
  });
} else {
  var congratsSound = new Sound('tada.mp3', Sound.MAIN_BUNDLE, error => {
    console.log('🚀 ~ congratsSound=newSound ~ error:', error);
  });
}

if (Platform.OS === 'android') {
  var tapSound = new Sound(require('../android/sounds/TapThr.mp3'));
} else {
  var tapSound = new Sound('punch.mp3', Sound.MAIN_BUNDLE, error => {
    console.log('🚀 ~ congratsSound=newSound ~ error:', error);
  });
}
...
const failSound = new Sound(require('../android/sounds/Fail.mp3'), error => {
  ...
  console.log(
    'Duration in seconds: ' +
      failSound.getDuration() +
      'number of channels: ' +
      failSound.getNumberOfChannels(),
  );
});
...
const adUnitId = __DEV__
  ...
  : Platform.OS == 'ios'
  ? TestIds.INTERSTITIAL
  : interstitialAdKeyAndroid;

const interstitialAd = InterstitialAd.createForAdRequest(adUnitId, {
  requestNonPersonalizedAdsOnly: true,
});

const LVLOne = ({navigation}) => {
  const {unlockedLevels, setUnlockedLevels, attemptsLeft, decreaseAttempts} =
    useLevel();
  ...
  const adRemovalPurchased = useAdRemovalPurchase();

  const showInterstitialAd = () => {
    if (interstitialAd.loaded && !adShown && !adRemovalPurchased) {
      interstitialAd.show();
      setAdShown(true);
    }
  };
  ...
  useEffect(() => {
    const eventLoadListener: any = interstitialAd.addAdEventListener(
      AdEventType.LOADED,
      () => console.log('add lodded'),
    );

    const eventErrorListener: any = interstitialAd.addAdEventListener(
      AdEventType.ERROR,
      error => {
        console.error('Ad failed to load: ', error);
      },
    );
    const adClosedListener: any = interstitialAd.addAdEventListener(
      AdEventType.CLOSED,
      () => {
  ...
  useEffect(() => {
    if (timerStarted) {
      if (points >= requiredTaps) {
        setMessage('Congratulations! You Beat Level 1!');
  ...
  const handleButtonClick = (buttonType: any) => {
    tapSound.stop();
    let isSameBut = !lastButtonPressed || lastButtonPressed !== buttonType;
    setLastButtonPressed(buttonType);
    if (points < requiredTaps && isSameBut) {
      setPoints(points + 1);
      tapSound.play();
    ...
    <ImageBackground
      ...
      <View style={styles.container}>
        ...
        <Text style={styles.infoText}>
          You can only earn points towards the progress bar if you hit the
          buttons right to left or left to right.
        </Text>
        <Text style={styles.infoText}>
          You need to make {requiredTaps - points} more taps to win!
        </Text>
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity
            ...
            <LinearGradient
              colors={['#FF6347', '#FFA500', '#4682B4']}
              style={{...styles.buttonGradient, width: '100%', height: '100%'}}>
        ...
        {addClosed && message && (
          <Modal visible={true} animationType="slide" transparent={true}>
            <View style={styles.modalContainer}>
              ...
              {message === 'Congratulations! You Beat Level 1!' && (
                <ConfettiCannon
                  count={200}
                  origin={{x: -20, y: 0}}
                  explosionSpeed={400}
