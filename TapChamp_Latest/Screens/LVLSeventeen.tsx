  ...
  useEffect(() => {
    if (timerStarted) {
      if (points >= requiredTaps && !adShown) {
        setMessage('Congratulations! You Beat Level 17!');
        congratsSound.play((success) => {
          if (!success) {
            console.log('Congrats sound did not play');
          }
        });
        if (!unlockedLevels.includes(18)) {
          setUnlockedLevels([...unlockedLevels, 18]);
        }
      showInterstitialAd();
        stopTimer();
    ...
    <Text style={styles.buttonText}>Tap!</Text>
        ...
        { (addClosed && message) && (
          ...
          <View style={styles.modalContainer}>
            ...
            {message === 'Congratulations! You Beat Level 17!' && (
              <ConfettiCannon 
                count={200} 
                origin={{ x: -20, y: 0 }} 
                explosionSpeed={400} 
