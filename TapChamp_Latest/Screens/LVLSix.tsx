  ...
  useEffect(() => {
    if (timerStarted) {
      if (points >= requiredTaps && !adShown) {
        setMessage('Congratulations! You Beat Level 6!');
        congratsSound.play((success) => {
          if (!success) {
            console.log('Congrats sound did not play');
          }
        });
        if (!unlockedLevels.includes(7)) {
          setUnlockedLevels([...unlockedLevels, 7]);
        }
        showInterstitialAd();
        stopTimer();
...
<TouchableOpacity style={styles.imageButton} onPress={() => handleButtonClick('T')}>
  ...
  <Modal visible={true} animationType="slide" transparent={true}>
    <View style={styles.modalContainer}>
      ...
      {message === 'Congratulations! You Beat Level 6!' && (
        <ConfettiCannon 
          count={200} 
          origin={{ x: -20, y: 0 }} 
          explosionSpeed={400} 
