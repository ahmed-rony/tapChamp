  ...
  useEffect(() => {
    if (timerStarted) {
      if (points >= requiredTaps && !adShown) {
        setMessage('Congratulations! You Beat Level 13!');
        congratsSound.play((success) => {
          if (!success) {
            console.log('Congrats sound did not play');
          }
        });
        if (!unlockedLevels.includes(14)) {
          setUnlockedLevels([...unlockedLevels, 14]);
        }
        showInterstitialAd();
        stopTimer();
    ...
    <Text style={styles.buttonText}>Tap!</Text>
        ...
        { (addClosed && message) && (
          ...
          <View style={styles.modalContainer}>
            ...
            {message === 'Congratulations! You Beat Level 13!' && (
              <ConfettiCannon 
                count={200} 
                origin={{ x: -20, y: 0 }} 
                explosionSpeed={400} 
