...
const LVLTwentyThree = ({ navigation }) => {
  ...
  useEffect(() => {
    if (timerStarted) {
      if (points >= requiredTaps && !adShown) {
        setMessage('Congratulations! You Beat Level 23!');
        congratsSound.play((success) => {
          if (!success) {
            console.log('Congrats sound did not play');
          }
        });
        if (!unlockedLevels.includes(24)) {
          setUnlockedLevels([...unlockedLevels, 24]);
        }
        showInterstitialAd();
        stopTimer();
    ...
    <Text style={styles.buttonText}>Tap!</Text>
        ...
        { (addClosed && message) && (
          ...
          <View style={styles.modalContainer}>
            ...
            {message === 'Congratulations! You Beat Level 23!' && (
              <ConfettiCannon 
                count={200} 
                origin={{ x: -20, y: 0 }} 
                explosionSpeed={400} 
