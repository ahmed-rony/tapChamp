...
if (Platform.OS === 'android') {
  var congratsSound = new Sound(require('../android/sounds/CongratsT.mp3'));
} else {
  var congratsSound = new Sound('tada.mp3', Sound.MAIN_BUNDLE);
}

if (Platform.OS === 'android') {
  var tapSound = new Sound(require('../android/sounds/TapThr.mp3'));
} else {
  var tapSound = new Sound('punch.mp3', Sound.MAIN_BUNDLE);
}
...
const LVLTwo = ({navigation}: any) => {
  ...
  useEffect(() => {
    const eventLoadListener = interstitialAd.addAdEventListener(
      AdEventType.LOADED,
      () => {
        console.log('Ad Loaded');
      },
    );
    const eventErrorListener = interstitialAd.addAdEventListener(
      AdEventType.ERROR,
      error => {
        console.error('Ad failed to load: ', error);
      },
    );
    const adClosedListener = interstitialAd.addAdEventListener(
      AdEventType.CLOSED,
      () => {
        interstitialAd.load();
        setAdClosed(true);
  ...
  useEffect(() => {
    if (timerStarted) {
      if (points >= requiredTaps && !adShown) {
        setMessage('Congratulations! You Beat Level 2!');
  ...
  return (
    <ImageBackground
      ...
      <Text style={styles.livesTextStyle}>
        Lives left: {String(attemptsLeft)}
      </Text>
        ...
        {addClosed && message && (
          <Modal visible={true} animationType="slide" transparent={true}>
            <View style={styles.modalContainer}>
              ...
              {message === 'Congratulations! You Beat Level 2!' && (
                <ConfettiCannon
                  count={200}
                  origin={{x: -20, y: 0}}
                  explosionSpeed={400}
